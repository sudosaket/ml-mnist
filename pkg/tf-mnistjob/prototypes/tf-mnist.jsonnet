...
local util = import "mypkgs/tf-mnistjob/util.libsonnet";
...
local tfJob = import "mypkgs/tf-mnistjob/tf-job.libsonnet";
...
local replicas = std.map(function(s)
                           s {
                             template+: {
                               spec+: {
                                  volumes: [ { name: "nfsvolume", persistentVolumeClaim: { claimName: "nfsmw" } } ] ,
